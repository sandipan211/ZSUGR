...
class CrossAttention(nn.Module):
    ...
    def forward(self, memory, clip_context):
        ...
        for _, layer in enumerate(self.layers):
            output = layer(output, clip_context)
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, memory, clip_context):
        ...
        att_E_output = self.cross_att_E(query=memory, key=clip_context, value=clip_context)[0] 
        ...
        att_P_output = self.cross_att_P(query=clip_context, key=memory, value=memory)[0]
