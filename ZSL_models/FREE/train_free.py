...
import csv
#import functions
import FREE.model as model
import FREE.util as util
import FREE.classifier as classifier_zero

# from FREE.config import opt
import time
import FREE.classifier_cls as classifier2
from FREE.center_loss import TripCenterLoss_min_margin,TripCenterLoss_margin
...
def train_FREE(opt):
    ...
    if opt.dataset in ['CUB','FLO', 'CADDY']:
    ...
    for epoch in range(0,opt.epochs):
        ...
        logger.info('[%d/%d]  Loss_D: %.4f Loss_G: %.4f, Wasserstein_dist:%.4f, vae_loss_seen:%.4f'% (epoch, opt.epochs, D_cost.item(), G_cost.item(), Wasserstein_D.item(),vae_loss_seen.item()))#,end=" ")
        netG.eval()
        netFR.eval()
        ...
        if best_zsl_acc <= gzsl_cls.acc_czsl:
            best_zsl_acc = gzsl_cls.acc_czsl
            best_czsl_model = gzsl_cls.best_model_czsl
            best_acc_per_class_czsl = gzsl_cls.acc_per_class_czsl

        logger.info(f'CZSL: {gzsl_cls.acc_czsl: 0.4f}')
        if best_gzsl_acc <= gzsl_cls.H:
            best_gzsl_epoch= epoch
            best_acc_seen, best_acc_unseen, best_gzsl_acc, best_model_gzsl, best_acc_seen_per_class, best_acc_unseen_per_class = \
        ...
        logger.info('GZSL: seen=%.3f, unseen=%.3f, h=%.3f' % (gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H))
            
        if epoch % 10 == 0:
            logger.info('GZSL: epoch=%d, best_seen=%.3f, best_unseen=%.3f, best_h=%.3f' % (best_gzsl_epoch, best_acc_seen, best_acc_unseen, best_gzsl_acc))
        # logger.info('ZSL: epoch=%d, best unseen accuracy=%.4f' % (best_zsl_epoch, best_zsl_acc))
        
        # reset G to training mode
        netG.train()
        netFR.train()

    # logger.info('feature(X+feat1): 2048+4096')
    logger.info('softmax: feature(X+feat1+feat2): 8494')
    ...
    torch.save(best_czsl_model, os.path.join(best_model_params_path,'best_czsl_model.pt'))
    torch.save(best_acc_per_class_czsl,os.path.join(best_model_params_path,'best_acc_per_class_czsl.pt'))
    if opt.gzsl:
        logger.info(f'Dataset {opt.dataset}')
        ...
        torch.save(netFR.state_dict(), os.path.join(last_chk_point_path,'last_chk_point_netFR.pt'))
        torch.save(netG.state_dict(), os.path.join(last_chk_point_path,'last_chk_point_netG.pt'))
        torch.save(best_model_gzsl, os.path.join(best_model_params_path,'best_gzsl_model.pt'))
