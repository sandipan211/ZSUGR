...
parser.add_argument('--dataset', default='CADDY', help='FLO')
parser.add_argument('--datadir', default='/workspace/arijit/sandipan/zsgr_caddy/hariansh/', help='path to dataset')
...
parser.add_argument('--gzsl', type=bool, required=True, help='enable generalized zero-shot learning')
...
parser.add_argument('--nclass_all', type=int, default=16, help='number of all classes')
parser.add_argument('--nclass_seen', type=int, default=10, help='number of all classes')
...
for epoch in range(opt.nepoch):
    ...
    if opt.gzsl: # Generalized zero-shot learning
        ...
        cls = classifier_embed_contras.CLASSIFIER(train_X, train_Y, netMap, opt.embedSize, data, nclass, opt.cuda,
                                                  ...
                                                  True)
        f.write('unseen=%.4f, seen=%.4f, h=%.4f \n' % (cls.acc_unseen, cls.acc_seen, cls.H))
        if cls.H > best_H:
            best_H = cls.H
            best_seen_acc = cls.acc_seen
            best_unseen_acc = cls.acc_unseen
    ...
    cls = classifier_embed_contras.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses), netMap,
                                                ...
                                                False)
    acc = cls.acc
    f.write('unseen class accuracy=%.4f \n'%acc)
    if cls.acc > best_czsl_acc:
        best_czsl_acc = cls.acc
...
f.write(f'Best CZSL acc: {best_czsl_acc}\n')
f.write(f'Best GZSL acc: {best_H} Best acc seen: {best_seen_acc} Best acc unseen: {best_unseen_acc}\n')
